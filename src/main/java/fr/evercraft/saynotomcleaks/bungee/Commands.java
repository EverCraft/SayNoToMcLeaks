...
public class Commands extends Command  implements Listener {
	...
	public void execute(CommandSender sender, String[] args) {
		if (!this.plugin.isEnabled()) {
			sender.sendMessage(new TextComponent(ChatColor.RED + "---------- [SayNoToMcLeaks v" + this.plugin.getDescription().getVersion() + " : By rexbut] ----------"));
		...
		if (args.length == 1 && args[0].equalsIgnoreCase("reload")) {
			...
			sender.sendMessage(new TextComponent(ChatColor.GREEN + "---------- [SayNoToMcLeaks v" + this.plugin.getDescription().getVersion() + " : By rexbut] ----------"));
